...
ELEMENT_FAMILY = {
    ...
    "B": "metalloid", "C": "nonmetal",
    "N": "pnictogen", "O": "chalcogen", "S": "chalcogen", "Se": "chalcogen", "Te": "chalcogen",
...
def select_diverse(df: pd.DataFrame, shortlist_size: int, max_per_signature: int) -> pd.DataFrame:
    selected = []
    counts: dict[str, int] = {}
    stoich_counts: dict[str, int] = {}
    ordered = df.sort_values(["screening_score", "distance_from_target_eV", "uncertainty_proxy_eV"])
    for _, row in ordered.iterrows():
        signature = row["chemistry_signature"]
        stoich = row["stoichiometry_class"]
        if counts.get(signature, 0) >= max_per_signature:
            continue
        if stoich_counts.get(stoich, 0) >= max(4, shortlist_size // 2):
            continue
        selected.append(row)
...
def write_report(shortlist: pd.DataFrame, source_rows: int, output_path: Path) -> None:
    lines = [
        ...
        "- Diversity controls: chemistry-family signature and stoichiometry class.",
...
def main() -> None:
    ...
    shortlist = select_diverse(df, args.shortlist_size, args.max_per_signature)
